...
cfgfile_path = os.path.join(directory, 'db_config.INI')
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        model = energyPATHWAYS.PathwaysModel(cfgfile_path, custom_pint_definitions_path, just_db=True)
        model.load_ini_into_db(sys.argv[1])
    else:
