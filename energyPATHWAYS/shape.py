...
class Shape(dmf.DataMapFunctions):    
    ...
    def geomap_to_primary_geography(self, attr='values', inplace=True):
        ...
        self.map_df_primary = cfg.geo.map_df(self.geography, ['time zone', self.primary_geography], normalize_as=self.input_type, map_key=geography_map_key)
        mapped_data = util.DfOper.mult((getattr(self, attr), self.map_df_primary), fill_value=np.nan)
