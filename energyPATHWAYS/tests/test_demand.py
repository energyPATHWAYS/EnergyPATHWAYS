...
class DemandTest(ut.TestCase):
    ...
    def test_demand_config(self):
        self.model.demand = Demand(self.model.cfgfile_path, self.model.custom_pint_definitions_path)
        self.model.configure_demand()
        ...
        self.assertEqual(len(demand.drivers), 36)
        ...
        households = demand.drivers[36]
        self.assertEqual(households.name, "households")
        self.assertEqual(households.geography_map_key, '2015 Total Households (ESRI)')
        self.assertEqual(households.base_driver_id, 1)
        ...
        housing_types = sorted(households.raw_values.index.get_level_values('housing types').unique().tolist())
        self.assertEqual(housing_types, [28, 29, 30])
        ...
        self.assertEqual(population.geography_map_key, '2015 Total Population (Esri)\r\n')
        ...
        divs = sorted(population.raw_values.index.get_level_values('census division').unique().tolist())
        self.assertEqual(divs, [56, 57, 58, 59, 60, 61, 62, 63, 64])
        self.assertEqual(population.raw_values.columns.values.tolist(), ['value'])
        self.assertEqual(len(population.raw_values), 549)
        self.assertEqual(population.raw_values.sum().value, 18682.459337480002 * population.unit_prefix)

        self.assertEqual(households.base_driver, demand.drivers[1])
