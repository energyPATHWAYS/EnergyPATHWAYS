...
class Demand(object):
    ...
    def calculate_demand(self):
        ...
        for sector in self.sectors.values():
            ...
            sector.set_default_shape(cfg.elect_default_shape_key)
    ...
    def aggregate_electricity_shapes(self, year, geomap_to_dispatch_geography=True, reconciliation_step=False, exclude_subsectors=None):
        ...
        denom = df.xs(0, level='timeshift_type').sum().sum() / len(shapes2.ShapeContainer.get_instance().cfg_weather_years)
    ...
    def get_weighted_feeder_allocation_by_sector(self):
        ...
        return feeder_allocation_by_sector.sort_index()
    ...
    def create_electricity_reconciliation(self):
        ...
        load_shape_base_year = shapes2.ShapeContainer.get_load_shape_base_year()
        ...
        top_down_shape = top_down_energy * shapes2.ShapeContainer.get_values(cfg.elect_default_shape_key)
    ...
    def stack_subsector_electricity_profiles(self, year):
        ...
        for sector in self.sectors.values():
            ...
            for subsector in sector.subsectors.values():
                ...
                df['year'] = year
                if aggregate_subsector_profiles_to_sector:
                    df = df.set_index(['sector', 'year'], append=True).sort_index()
                else:
                    df['subsector'] = subsector.name
                    df = df.set_index(['sector', 'subsector', 'year'], append=True).sort_index()
                df = df.reorder_levels(index_levels).sort_index()
                sector_stack.append(df)
            if aggregate_subsector_profiles_to_sector:
                sector_stack = [util.DfOper.add(sector_stack)]
            stack += sector_stack
        stack = pd.concat(stack).sort_index()
...
class Subsector(schema.DemandSubsectors):
    def __init__(self, name, drivers, stock, service_demand, energy_demand, service_efficiency, scenario):
        ...
        self.shapes_weather_year = int(np.round(np.mean(shapes2.ShapeContainer.get_active_dates_index().year)))
        self.electricity_reconciliation = None
    ...
    def return_shape_after_flex_load(self, shape_values, percent_flexible, max_lag_hours, max_lead_hours, percent_of_energy_reconciled):
        ...
        flex = shapes2.ShapeObj.produce_flexible_load(shape_df, percent_flexible=percent_flexible, hr_delay=max_lag_hours, hr_advance=max_lead_hours)
        return flex
    ...
    def aggregate_subsector_electricity_shape(self, year):
        ...
        if has_flexible_load:
            ...
            if flexible_load_tech_index:
                ...
                remaining_shape = util.DfOper.mult((shapes2.ShapeContainer.get_values(active_shape), remaining_energy), collapsible=False)
                remaining_shape = util.DfOper.add((remaining_shape, inflexible_tech_load), collapsible=False)
            ...
            remaining_shape = util.DfOper.mult((shapes2.ShapeContainer.get_values(active_shape), remaining_energy))
    ...
    def group_output(self, output_type, levels_to_keep=None, specific_years=None):
        ...
        if output_type=='energy':
            ...
            if self.sub_type != 'link':
                ...
                if df is not None:
                    df['unit'] = cfg.getParam('mass_unit', section='UNITS')
                    df.set_index('unit',append=True,inplace=True)
                return df
            else:
                return None
