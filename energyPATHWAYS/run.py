...
def click_run(path, config, scenario, load_demand, solve_demand, load_supply, solve_supply, load_error, export_results, pickle_shapes, save_models, log_name, api_run, clear_results, subfolders):
    run(path, config, scenario, load_demand, solve_demand, load_supply, solve_supply, load_error, export_results, pickle_shapes, save_models, log_name, api_run, clear_results, subfolders)

def run(path, config, scenarios, load_demand=False, solve_demand=True, load_supply=False, solve_supply=True, load_error=False,
        export_results=True, pickle_shapes=True, save_models=True, log_name=None, api_run=False, clear_results=False, subfolders=False):
    global model
    ...
    for scenario in scenarios:
        ...
        model = load_model(load_demand, load_supply, load_error, scenario, api_run)
        if not load_error:
            model.run(scenario,
...
def load_model(load_demand, load_supply, load_error, scenario, api_run):
...
#        with open(os.path.join(cfg.workingdir, 'dispatch_class.p'), 'rb') as infile:
        ...
        model = PathwaysModel(scenario, api_run)
    return model
