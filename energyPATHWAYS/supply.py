    ...
    def reconcile_oversupply(self,year,loop):    
        ...
        for node in self.nodes.values():
           ...
           if oversupply_factor is not None:
               if cfg.rio_supply_run and node.id not in cfg.rio_excluded_nodes:
                   continue
...
#            return        
    ...
    def calculate_potential_constraints(self, year):
        ...
        if hasattr(self,'potential') and self.potential.data is True and self.enforce_potential_constraint == True and not cfg.rio_supply_run:
