                ...
                if hasattr(node,'active_dispatch_costs'):
                    ...
                    for group in groups:
                        ...
                        try:
                            ...
                            pdb.set_trace()
                    self.active_thermal_dispatch_df = self.active_thermal_dispatch_df[((np.array([int(x[-6:-2]) for x in self.active_thermal_dispatch_df.index.get_level_values('thermal_generators')])==year) == True) | ((self.active_thermal_dispatch_df[year]>0) == True)]
