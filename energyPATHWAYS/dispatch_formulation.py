...
def ld_formulation(dispatch):    
    ...
    def net_transmit_power_by_geo_rule(model, geography, timepoint):
        # line is constructed as (from, to)
        # net transmit power = sum of all imports minus the sum of all exports
        return model.Net_Transmit_Power_by_Geo[geography, timepoint] == sum([model.Transmit_Power[str((g, geography)), timepoint] for g in model.GEOGRAPHIES if g!=geography])/1.05 - \
                                                                        sum([model.Transmit_Power[str((geography, g)), timepoint] for g in model.GEOGRAPHIES if g!=geography]) *1.05
    ...
    def total_imbalance_penalty_rule(model):
        return sum((model.Upward_Imbalance[g, t] * model.upward_imbalance_penalty +
